...
let albums = [
  ...
  { name: "OLDEES", cover: "card3.webp", desc: "Golden classics.", songs: ["Woh Shaam Kuch Ajeeb Thi.mp3", "O Mere Dil Ke Chain.mp3", "Pukarta Chala Hoon Main.mp3"] }
];
...
albums.forEach(album => {
  ...
  ["img", "h3", ".desc"].forEach(sel => {
    card.querySelector(sel).addEventListener("click", () => loadAlbum(album));
  });
  ...
  card.querySelector(".play").addEventListener("click", e => {
    ...
    else { loadAlbum(album); playmusic(songs[0]); currentsong.play(); updatePlayIcon(true); }
  });
...
function loadAlbum(album) {
  ...
  songs.forEach(song => {
    ...
    li.innerHTML = `<div class="info"><div>${decodeURIComponent(song).replace(".mp3", "")}</div><div>${album.name}</div></div>`;
...
function highlightCurrentSong() {
  document.querySelectorAll(".song-list li").forEach(li => { li.classList.remove("active"); if (li.dataset.track === currentTrack) li.classList.add("active"); });
}

function formatTime(sec) { let m = Math.floor(sec / 60), s = Math.floor(sec % 60); return `${m}:${s.toString().padStart(2, "0")}`; }
...
function updatePlayIcon(isPlaying) {
  ...
  document.querySelectorAll(".card .play path").forEach(p => {
    if (currentAlbum && p.parentElement.parentElement.querySelector("h3").textContent === currentAlbum.name) {
...
// Seekbar
document.querySelector(".seekbar").addEventListener("input", e => { currentsong.currentTime = e.target.value; });
